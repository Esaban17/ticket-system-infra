...
export function LoginPage() {
  ...
  // Carga el proveedor de auth: decide si se muestra el form mock y/o el SSO.
  useEffect(() => {
    getAuthConfig()
      .then(setAuthConfig)
      .catch(() => setAuthConfig({ provider: 'mock', cognito: null }));
  }, []);

  const cognito = authConfig?.cognito ?? null;
  // Con AUTH_PROVIDER=cognito se oculta el login por contraseña (solo SSO).
  const showPasswordForm = authConfig?.provider !== 'cognito';
      ...
      <main className="flex-grow flex flex-col items-center justify-center px-4 py-12">
        ...
        <div className="w-full max-w-[440px] bg-white border border-slate-200 shadow-sm rounded-sm p-8">
          ...
          <form className="space-y-5" onSubmit={handleSubmit} noValidate>
            ...
            {/* SSO Cognito: habilitado cuando hay config; si no, placeholder deshabilitado. */}
            {cognito ? (
              <Button
                type="button"
                variant="secondary"
                ...
                onClick={() => window.location.assign(cognitoLoginUrl(cognito))}
              >
              ...
              <span
                ...
                title="SSO disponible cuando Cognito está configurado (EP-14)"
              >
                <Button
                  type="button"
                  variant="secondary"
