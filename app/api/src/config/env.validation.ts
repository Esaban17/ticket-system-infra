...
export const envSchema = z
  .object({
    ...
    // Base de datos.
    // D5-B: DATABASE_URL es OPCIONAL aquí porque el secret-loader la compone en
    // tiempo de arranque (antes de validar) a partir del secret de Secrets Manager
    // (SECRET_ARN) + DB_HOST/DB_PORT/DB_NAME del ConfigMap. El refine de abajo
    // exige que, tras el arranque, exista DATABASE_URL o bien SECRET_ARN.
    DATABASE_URL: z.string().url('DATABASE_URL debe ser una URL válida de PostgreSQL').optional(),

    // D5-B: ARN del secret con las credenciales de BD (Secrets Manager). Presente
    // en cluster; ausente en local (se usa DATABASE_URL del .env). DB_HOST/DB_PORT/
    // DB_NAME son no sensibles y los inyecta el ConfigMap.
  ...
  .superRefine((env, ctx) => {
    // D5-B: o bien DATABASE_URL ya está compuesto (loader ya corrió, o .env local),
    // o bien SECRET_ARN está presente para que el loader la componga al arrancar.
    if (!env.DATABASE_URL && !env.SECRET_ARN) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['DATABASE_URL'],
        message: 'Falta DATABASE_URL (y no hay SECRET_ARN para derivarla desde Secrets Manager).',
      });
    }
  });
